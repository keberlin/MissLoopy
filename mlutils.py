...
def Authenticate(cookies=None, remote_addr=None):
    ...
    # Authenticate
    entry = (
        db.session.query(ProfileModel)
        .filter(ProfileModel.id == id, ProfileModel.email == email, ProfileModel.password == password)
        .one_or_none()
    )
    if not entry:
        return None
...
def Blocked(id, id_with):
    count = (
        ...
        .select_from(BlockedModel)
        .filter(BlockedModel.id == id, BlockedModel.id_block == id_with)
        .scalar()
    )
    return count > 0
...
def BlockedMutually(id, id_with):
    count = (
        ...
        .filter(
            or_(
                and_(BlockedModel.id == id, BlockedModel.id_block == id_with),
                and_(BlockedModel.id == id_with, BlockedModel.id_block == id),
            )
        )
        .scalar()
    )
...
def PurgeResults(id_search):
    ...
    for entry in entries:
        ...
        if id_previous:
            db.session.query(ResultModel).filter(ResultModel.id == id, ResultModel.id_search == id_previous).update(
                {"id_next": id_next}, synchronize_session=False
            )
        if id_next:
            db.session.query(ResultModel).filter(ResultModel.id == id, ResultModel.id_search == id_next).update(
                {"id_previous": id_previous}, synchronize_session=False
            )
        db.session.commit()
