...
class IPAddressIPValidationError(Error):
    ...
    def __str__(self):
        return "%s is not a valid address (hint, it's probably a network)" % (repr(self._ip))
...
def collapse_address_list(addresses):
    ...
    for ip in addresses:
        if isinstance(ip, BaseIP):
            if ips and ips[-1]._version != ip._version:
                raise IPTypeError(
                    "Can only collapse like-versioned objects -"
                    " v%d: %s, v%d %s" % (ips[-1]._version, str(ips[-1]), ip._version, str(ip))
                )
            ips.append(ip)
    ...
    return _collapse_address_list_recursive(sorted(addrs + nets, key=BaseNet._get_networks_key))
...
class BaseIP(IPAddrBase):
    ...
    def __eq__(self, other):
        try:
            return not (self._ip != other._ip or self._version != other._version)
...
class BaseNet(IPAddrBase):
    ...
    def __eq__(self, other):
        try:
            return self._version == other._version and self._ip == other._ip and int(self.netmask) == int(other.netmask)
        except AttributeError:
            return NotImplemented
    ...
    def __contains__(self, other):
        ...
        if isinstance(other, BaseNet):
            return int(self.network) <= int(other._ip) and int(self.broadcast) >= int(other.broadcast)
    ...
    def address_exclude(self, other):
        ...
        if not self._version == other._version:
            raise IPTypeError("%s and %s aren't of the same version" % (str(self), str(other)))
        ...
        while s1 != other and s2 != other:
            if other in s1:
                ...
                raise IPAddressExclusionError(
                    "Error performing exclusion: " "s1: %s s2: %s other: %s" % (str(s1), str(s2), str(other))
                )
        if s1 == other:
            ret_addrs.append(s2)
        elif s2 == other:
    ...
    def subnet(self, prefixlen_diff=1, new_prefix=None):
        ...
        if not self._is_valid_netmask(str(new_prefixlen)):
            raise PrefixlenDiffInvalidError(
                "prefix length diff %d is invalid for netblock %s" % (new_prefixlen, str(self))
            )
    ...
    def supernet(self, prefixlen_diff=1, new_prefix=None):
        ...
        if self.prefixlen - prefixlen_diff < 0:
            raise PrefixlenDiffInvalidError(
                "current prefixlen is %d, cannot have a prefixlen_diff of %d" % (self.prefixlen, prefixlen_diff)
            )
