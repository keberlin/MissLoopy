...
for i in range(1, n + 1):
    ...
    session1.execute(
        "INSERT INTO profiles (created2,verified,x,y,name,email,dob,gender,ethnicity,gender_choice,ethnicity_choice) VALUES ("
        + Quote(str(now))
        + ",1,"
        + str(x)
        + ","
        ...
        + str(gender_choice)
        + ","
        + str(ethnicity_choice)
        + ")"
    )
    if i % (n / 100) == 0:
        print(
            "Processed " + str(i * 100 / n) + "%..\r",
        )
session1.commit()
