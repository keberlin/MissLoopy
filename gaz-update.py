  ...
  with open(file,'rb') as csvfile:
    ...
    for row in reader:
      ...
      if population > entry.population:
        session.query(LocationModel).filter(LocationModel.location==location).update({"x":x,"y":y,"tz":tz,"population":population},synchronize_session=False)
        session.commit()
        updated += 1
        continue
