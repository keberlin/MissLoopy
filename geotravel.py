...
def fetch(beg, end):
    ...
    url = "https://maps.googleapis.com/maps/api/directions/json?origin=%s&destination=%s&region=uk&key=%s" % (
        str(lat1) + "," + str(lng1),
        str(lat2) + "," + str(lng2),
        SERVER_KEY,
    )
...
def geotravel(beg, end, accurate=False):
    ...
    sql = "INSERT INTO geotravel (start,destination,travelling_time,distance,modified) VALUES (%s,%s,%s,%s,%s)" % (
        Quote(beg),
        Quote(end),
        str(seconds),
        str(distance),
...
def geotravel_update(n):
    ...
    for entry in db.fetchall():
        ...
        sql = "UPDATE geotravel SET travelling_time=%s,distance=%s,modified=%s WHERE start=%s AND destination=%s" % (
            str(seconds),
            str(distance),
            Quote(modified),
            Quote(beg),
